...
/**
 * @title PizzaShapesSettlementV2
 * @notice Handles match entry fees and prize distribution with charity support
 ...
 */
contract PizzaShapesSettlementV2 is PizzaUUPSBase {
    ...
    uint256 public constant TIER_1 = 0.25 ether;    // $0.25 worth of PIZZA
    uint256 public constant TIER_2 = 0.50 ether;    // $0.50 worth of PIZZA
    uint256 public constant TIER_3 = 1.00 ether;    // $1.00 worth of PIZZA

    // Burn address - tokens sent here are permanently removed from circulation
    // Using 0xdead is a common convention; tokens can never be recovered
    ...
    // Events
    event MatchCreated(bytes32 indexed matchId, uint8 tier, uint256 entryAmount);
    ...
    function enterMatch(bytes32 matchId, uint8 tier) external whenNotPaused {
        ...
        if (m.createdAt == 0) {
            ...
            emit MatchCreated(matchId, tier, amount);
        } else {
            if (m.settled) revert MatchAlreadySettled();
            if (m.entryAmount != amount) revert TierMismatch();
            if (m.players.length >= _getMaxPlayers(tier)) revert MatchFull();
        }
    ...
    function settleMatch(
        ...
        // Sending to 0xdead effectively removes tokens from circulation
     ...
     * @dev Called by OPERATOR_ROLE at end of each week
     * @param winners Top 3 player addresses (in order: 1st, 2nd, 3rd)
     * @param percentages Distribution percentages [50, 30, 20] = [50%, 30%, 20%]
     */
    function distributeWeeklyTop3(
        ...
        for (uint256 i = 0; i < 3; i++) {
            if (winners[i] != address(0)) {
                ...
                // Note: weeklyVault must approve this contract
     ...
     * @dev Called by OPERATOR_ROLE when a player wins the free roll
     * @param winner Address of the free roll winner
     * @param amount Amount to pay out (1/6 of daily pool)
     */
    function payFreeRollWinner(
        ...
        // Transfer from vault to winner
        // Note: freeRollVault must approve this contract
    ...
    /**
     * @notice Get max players per tier
     */
    function _getMaxPlayers(uint8 tier) internal pure returns (uint256) {
        if (tier == 1) return 2;  // 4x4 board: 2 players
        if (tier == 2) return 4;  // 6x6 board: up to 4 players
        if (tier == 3) return 6;  // 8x8 board: up to 6 players
        return 2;
    }

    /**
     * @notice Get tier amount (external view)
     */
    function getTierAmount(uint8 tier) external pure returns (uint256) {
        return _getTierAmount(tier);
    }
    ...
    /**
     * @notice Get all charity wallet addresses
     */
    ...
    /**
     * @notice Calculate prize breakdown for a given pool
     */
     ...
     * @dev Only for recovering tokens accidentally sent to contract
     */
